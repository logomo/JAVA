...
class Result {
    ...
    // Write your code here
    	...
    	int counter = 0;
    	char head = worker[0];
    	while (point < worker.length) {
    		char challenger = worker[point];
    		if (challenger != head) {
    			head = challenger;
    		}
    		else {
...
public class Solution {
    public static void main(String[] args) throws IOException {
    	Path inputPath = Paths.get("src\\Inputs\\StringManipulation02\\input.txt");
		Path outputPath = Paths.get("src\\Inputs\\StringManipulation02\\output.txt");
